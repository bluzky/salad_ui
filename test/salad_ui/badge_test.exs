defmodule SaladUI.BadgeTest do
  ...
  describe "Test Badge" do
    ...
    test "It renders a badge default variant" do
      ...
      for class <- ~w(inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary text-primary-foreground hover:bg-primary/80) do
        assert html =~ class
      end

      assert html =~ "<div class=\"inline-flex px-2.5 py-0.5 rounded-full"
      assert html =~ "Badge"
    end
