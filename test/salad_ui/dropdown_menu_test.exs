defmodule SaladUI.DropdownMenuTest do
  ...
  describe "Test Dropdown menu" do
    ...
    test "It renders dropdown menu content correctly" do
      ...
      for css_class <- ~w(z-50 animate-in peer-data-[state=closed]:fade-out-0 peer-data-[state=open]:fade-in-0 peer-data-[state=closed]:zoom-out-95 peer-data-[state=open]:zoom-in-95 peer-data-[side=bottom]:slide-in-from-top-2 peer-data-[side=left]:slide-in-from-right-2 peer-data-[side=right]:slide-in-from-left-2 peer-data-[side=top]:slide-in-from-bottom-2) do
        assert html =~ css_class
      end
    end
